goosh.command = function() {
			...
			if (tokens[0] != "set" && tokens[0] != "settings")
			{
				...
				while (goosh.config.urls[j]) // replace search result numbers
				{
					if (tokens[i] == j)
					{
						...
						if (i == 0)
							args.push("open"); // number shortcut
					}
	...
	// more
	if (searcher.more && args.length>0)
		this.config.moreobj = searcher;
...
goosh.getquery = function() {
	...
	//{window.location.href="http://gshell.grothkopp.com/#"+goosh.lib.get("q"); return;} //query = decodeURI(goosh.lib.get("q"));
